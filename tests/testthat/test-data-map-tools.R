...
### bad data_in
## [x] missing code_in in data_in
bad_data_in$NA_code_in <- good_data_in %>%
  dplyr::mutate(std_A = dplyr::case_when(std_A == "x3333" ~ NA_character_,
                                         TRUE ~ std_A))
bad_dm$value_no_code_in <- bad_data_in$NA_code_in %>%
  dplyr::left_join(good_weights, by = "std_A")

### bad codes/weights
## [ ] extra codes with full join //
...
# [x] missing code-out for code-in
...
### bad values
bad_dm$codes_no_weights <-
  good_dm %>%
  select(-weight)

bad_dm$one2many_NA_value <-
  conformr:::toy_AB$data_map %>%
  filter(n_dest > 1, country == "AUS") %>%
  select(std_A, std_B, weight, A_100) %>%
  mutate(A_100_NA = case_when(std_B %in% c("C8", "C9") ~ NA_real_,
                              TRUE ~ A_100))

bad_dm$oneMany2one_NA_value <-
  conformr:::toy_AB$data_map %>%
  filter(n_dest == 1, country == "AUS") %>%
  select(std_A, std_B, weight, A_100) %>%
  mutate(A_100_NA = case_when(std_A %in% c("x1111", "x3333") ~ NA_real_,
                              TRUE ~ A_100))


# data_map grouped checks ----
...
test_that("missing codes raise error in dm_check_weights()", {
  expect_error(dm_check_weights(bad_dm$code_in_no_out, std_A, std_B))
  expect_error(dm_check_weights(bad_dm$value_no_code_in, std_A, std_B))
})
