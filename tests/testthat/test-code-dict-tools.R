...
test_that("incomplete `code_dict` raises check_cd_coverage() warning", {
  expect_warning(check_cd_coverage(data = data_in, code_dict = dropA_dict, code_in = "code_A"))
})
...
test_that("make_cd_equal output matches test dict", {
  fnc_out <- make_cd_equal(dup_codes, code_in = code_A, code_out = code_B, name_weight_col = "weight")
