...
#'
#' @param x
#'   * For `as_pairs_from_named()`: a named vector or nested list.
#'   * For `pairs_to_named` fncs: a data frame
...
#'
#' @return
#'   * For `as_pairs_from_named()`: a two-coloumn tibble
...
#'   pairs_to_named_vector(veg_pairs)
#' identical(veg_vec, veg_from_pairs)
#' 
#' animal_pairs <- as_pairs_from_named(animal_list, "class", "animal") |> 
...
#' @export
pairs_to_named_vector <- function(x, names_from = name, values_from = value){
  ordered_cols <- dplyr::select(x, {{names_from}}, {{values_from}})
  tibble::deframe(ordered_cols)
}

#' @describeIn as_pairs Convert column pairs to nested named list
#' @export
pairs_to_named_list <- function(x, names_from = name, values_from = value){
  nested_cols <- dplyr::select(x, {{names_from}}, {{values_from}}) |>
