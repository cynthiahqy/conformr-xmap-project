...
```{r fnc-add-weights, send_to="R/add_weights.R"}
...
#' @importFrom xmap abort_dup_pairs
...
mock$xmap_abc |>
  as_tibble() |>
  select(lower, upper) |>
  add_weights_placeholder(from = lower, to = upper, frac_symbol = "?", unit_symbol = "1") |>
  summarise_target0.data.frame(from = lower, to = upper, weights = weights_p)
```
