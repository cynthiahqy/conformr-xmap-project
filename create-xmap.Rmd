...
vhas_xmap_props <- function(v_from, v_to, v_weights){
  ...
  v_lengths <- .calc_vector_lengths(v_from, v_to, v_weights)
...
verify_named_as_recode_unique <- function(x){
  stopifnot(is.vector(x))
  set_name <- unique(names(x))
  set_value <- unique(unlist(unname(x)))
  stopifnot(length(set_name) == length(set_value))
  invisible(x)
}

#' @describeIn verify_named Verify named vector or list has only one-to-one relations
verify_named_all_1to1 <- verify_named_as_recode_unique

#' @describeIn verify_named Verify named vector or list has no duplicate names
verify_named_no_dup_names <- function(x){
  dup_idx <- anyDuplicated(names(x))
  stopifnot(!as.logical(dup_idx))
  invisible(x)
}

#' @describeIn verify_named Verify named vector or list has no duplicate values (after unnesting)
verify_named_no_dup_values <- function(x){
