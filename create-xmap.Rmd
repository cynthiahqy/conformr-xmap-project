...
#'
#' @param x
#' @param new_col_weights 
#'
#' @return
#' @export
#'
#' @examples
xmap_reverse.xmap_df <- function(x, new_col_weights = "r_weights"){
  stopifnot(inherits(x, "xmap_df"))
  x_attrs <- attributes(x)
  df <- as.data.frame(x)
  
  ## check xmap can be reversed
  x_to <- df[[x_attrs$col_to]]
  ...
  ## make new xmap
  df[[new_col_weights]] <- 1
  new_from <- x_attrs$col_to
  new_to <- x_attrs$col_from
  new_weights <- new_col_weights
