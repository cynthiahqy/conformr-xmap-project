...
Only checks base object and attribute types.
...
-   validates data.frame element types:
    -   source, target, weights columns are present, and other columns can be dropped
    -   source/target columns are / can be coerced into factors
    -   weights is numeric
-   checks for:
    -   duplicate source-target rows (weights should be collapsed into a single row)

    -   `NA` values in the weights column

    -   `sum()` of values in weights column by source-code is `1` (or close..?)
