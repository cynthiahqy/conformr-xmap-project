...
from_degree <- mock$xmap_abc |>
  group_by(lower) |>
  mutate(from_degree = n()) |>
  ungroup() |>
  # group_by(upper) |>
  # mutate(to_degree = n()) |>
  # ungroup() |>
  arrange(desc(from_degree))

from_degree |>
  select(lower, upper, share) |>
  reactable::reactable(groupBy = "lower")
...
mock$xmap_abc |>
  as_tbl_graph() |>
  activate(nodes) |>
  mutate(layer = case_when(name %in% disjoint_abc$from_set ~ 1,
...
mock$xmap_abc |>
  as_tibble() |>
  group_by(upper) |>
  mutate(share_char = case_when(share == 1 ~ "",
...
### Helper: `add_weights_placeholder()`
...
mock$xmap_abc |>
  as_tibble() |>
  select(lower, upper) |>
  add_weights_placeholder(from = lower, to = upper, frac_symbol = NA, unit_symbol = "1")
```

### Mock-Up: Summarise

```{r}
summarise_target0.data.frame <- function(
    df, from, to, weights,
...
#'
#' @export
print_target_summary <- function(links, from, to, weights, composition_to, collapse, frac_formula, unit_formula){
...
print_target_summary.data.frame <- function(
...
#' @export
#' @rdname summarise_target
print_target_summary.xmap_df <- function(links,
                                     composition_to = "parts_{{from}}",
                                     collapse = "+",
                                     frac_formula = "{from}*{weights}",
                                     unit_formula = "{from}"
...
mock$xmap_abc |> as.data.frame() |>
  print_target_summary.data.frame(from = lower, to = upper, weights = share)
...
### Integration with ggraph & igraph layouts via tidygraphs

#### Bipartite Layout

See <https://www.cynthiahqy.com/posts/xmap-ggraph-bipartite/> for details on bipartite layout
...
litr::add_vignettes(c("../xmap-source-files/making-xmaps.Rmd",
