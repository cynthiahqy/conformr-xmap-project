...
.get_link_types.xmap_df <- function(xmap_df){
  ...
  if (has_recode_only(x_weights, x_to)) {
    type <- "recode only"
  } else if (has_collapse_recode_only(x_weights, x_to)) {
    type <- "recode and collapse only"
  } else if (has_split_recode_only(x_weights, x_to)) {
    type <- "recode and split only"
  } else {
    flags <- c(
