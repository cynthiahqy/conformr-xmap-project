...
### Conversion: `xmap_to_matrix()`

```{r}
#' Extract incidence matrix from xmap objects
#' 
#' Transforms `xmap` objects into incidence matrix.
#' 
...
```{r}
add_files_to_folder <- function(files, folder){
  fs::dir_create(folder)
  mapply(fs::file_copy, files, folder)
}
add_files_to_folder("../xmap-source-files/using-xmaps.bib", "vignettes")
```

```{r}
## works
vignette_images <- c("../xmap-source-files/plot-coverage-matrix.png",
                     "../xmap-source-files/plot-missing-val-bigraph.png",
                     "../xmap-source-files/plot-weight-sum-matrix.png")
add_files_to_folder(vignette_images, "vignettes")
```
