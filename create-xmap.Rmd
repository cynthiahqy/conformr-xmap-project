...
new_xmap_df <- function(x = data.frame(), col_from, col_to, col_weights, from_set = NULL) {
  ...
  class(x) <- union(c("xmap_df", "xmap"), class(x))
  structure(x,
    col_from = col_from,
    col_to = col_to,
...
### Error Helpers (`validate-helpers.R`)
...
```{r fnc-check-weights, send_to="R/validate-helpers.R"}
...
modify_xmap_df <- function(xmap_df, from = NULL, to = NULL, weights = NULL){
  ...
  col_from <- ifelse(is.null(rlang::maybe_missing(from)),
                     x_attrs$col_from,
                     deparse(substitute(from)))
  col_to <- ifelse(is.null(rlang::maybe_missing(to)), 
                     x_attrs$col_to,
                     deparse(substitute(to)))
  col_weights <- ifelse(is.null(rlang::maybe_missing(weights)), 
                     x_attrs$col_weights,
...
modify_xmap_df(iso_xmap, from = country, to = ISONumeric)
```
...
```{r}
xmap_reverse.xmap_df <- function(xmap_df, .weights_to = "r_weights"){
}
```
...
### Helper: xmap type classification
...
tibble::as_tibble(valid_xmap_df) |>
  as_xmap_df(from = sector, to = new_sec, weight) |>
  print.xmap_df()
