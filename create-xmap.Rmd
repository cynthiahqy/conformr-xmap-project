...
### `xmap_tbl` subclass properties

Has all the same properties as `xmap_df` and is also a tibble.
...
new_xmap_df <- function(x, col_from, col_to, col_weights, from_set = NULL) {
  ...
  class(x) <- c("xmap_df", "xmap", "data.frame")
  structure(x,
    col_from = col_from,
    col_to = col_to,
...
### `xmap_tbl` Construction

Has all the same properties as `xmap_df` except it is also a tibble.
...
new_xmap_tbl <- function(x, col_from, col_to, col_weights, from_set = NULL) {
  ...
  class(x) <- c("xmap_tbl", "xmap_df", "xmap", "tbl_df", "tbl", "data.frame")
...
testthat::test_that(
  "new_xmap_tbl() accepts arbitrary tibbles with correct from argument",
  {
    ...
    xmap_attrs <- attributes(xmap)
    testthat::expect_s3_class(xmap, "xmap_tbl")
    testthat::expect_identical(xmap_attrs$col_from, "x")
    testthat::expect_identical(xmap_attrs$col_to, "y")
    testthat::expect_identical(xmap_attrs$col_weights, "z")
    testthat::expect_identical(xmap_attrs$from_set, unique(df$x))
...
```{r}
#' @describeIn as_xmap Coerce a `data.frame` to `xmap_df`
#'
...
#' as_xmap(links, from = a, to = b, weights = w)
as_xmap.data.frame <- function(x, from, to, weights, .keep_all = TRUE) {
  ## coercion & checks
  stopifnot(is.data.frame(x))
...
#'
#' @examples
xmap_reverse <- function(x, weights_to = "r_weights"){
  UseMethod("xmap_reverse")
}

#' @describeIn xmap_reverse
#' @return xmap_df
#' 
#' @export
...
```{r}
testthat::test_that("xmap_reverse() works as expected",             {
  x <- tibble::tribble(
      ~from, ~to, ~weights,
      "A1", "B01", 1,
      "A4", "B03", 0.25,
  ...
  testthat::expect_identical(xmap_reverse.xmap_df(x), x_rev)
  testthat::expect_identical(df_check_reversible(x,"to"), x)
}
)
```
...
valid_xmap_df <-
  ...
  as_xmap(from = sector, to = new_sec, weight)

print.xmap_df(valid_xmap_df)
```
