...
The utility of the tidy nomenclature format is most evident when we look at transforming a nomenclature variable into a target nomenclature as a set of matrix operations.

Let $\bf{B}$ be the $n \times m$ incidence matrix describing the graph representation of a valid crossmap. $\bf{B}$ is row indexed by the source nomenclature, and column indexed by the target nomenclature. For example, we can take a `xmap_df` and convert it into a matrix:
...
Notice that we can re-express the two conditions for a valid crossmap in terms of properties of the incidence matrix $\bf{B}$:
...
#### Comparison to Iterative or Case-Wise code

Contrast this to a more traditional iterative or case-wise approach to implementing a nomenclature transformation. Regardless of whether you code the transformation as a distribution of value from the source nomenclature nodes or collection of value by the target nomenclature nodes, there are only ad-hoc ways of validating node-by-node transformations.
...
To illustrate, consider the source node `f` in the `origin` source nomenclature. It is split by three outgoing links to `DD`, `EE` and `FF`. A case-wise approach to coding this transformation might look something like:
...
Notice that there is no simple way to check that the three weights sum to one.

Now, consider the target node `AA` which is in a collapse relation with `a`, `b` and `c`. An iterative approach might look something like:
...
Notice that this requires updating the value for `AA` and makes it difficult to see at a glance which source nodes feed into it.

Now consider combining multiple relation types in a single transformation. Such transformations might be implemented using a combination of case-wise and iterative code making it even harder to audit the transformation and identify errors.
