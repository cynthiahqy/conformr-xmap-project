...
-   **one-from-many**: a set of links where a single target node is linked to multiple source nodes. This "collpase" relation is more commonly known as a **many-to-one**. Links can have either unit or fractional weights depending on whether the source nodes are part of one-to-one or one-to-many relations.
...
This approach can help identify mistakes such as accidentally assigning a value twice:

```{r}
fruit_color_mistake <- c(fruit_colors, pear = "green")
...
student_group_mistake <- list(GRP1 = c("kate", "jane", "peter"),
                           GRP2 = c("terry", "ben", "grace"),
                           GRP3 = c("cindy", "lucy", "kate" ))

student_group_mistake |>
  verify_named_no_dup_values()
```

Verification as crossmaps requires adding weights and is directional:
