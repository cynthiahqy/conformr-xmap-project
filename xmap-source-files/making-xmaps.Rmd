...
simple_xmap <- simple_x |>
  as_xmap(from = source, to = target, weights = share)
...
iso_xmap <- iso_codes |>
  dplyr::mutate(link = 1) |>
  as_xmap(from = ISONumeric, to = ISO2, weights = link, .keep_all = TRUE)
```
...
agg_xmap <- agg_x |>
  dplyr::mutate(link = 1) |>
  as_xmap(from = adm1, to = ctr, weights = link)
...
state_xmap <- state_data |>
  dplyr::mutate(ctr = "AU",
                adm1,
                share = Pop / sum(Pop)) |>
  as_xmap(from = ctr, to = adm1, weights = share, .keep_all = FALSE)
...
canada_xmap <- dplyr::bind_rows(canada_unit, canada_frac) |>
  as_xmap(from = NAICS1997, to = ISIC3, weights = share)
...
dplyr::bind_rows(canada_unit, canada_frac) |>
  as_xmap(from = ISIC3, to = NAICS1997, weights = share)
```
...
```{r}
iso_xmap |>
  as_xmap(from = ISO2, to = ISONumeric, weights = link)
```
...
```{r}
iso_xmap |>
  as_xmap(from = ISO2, to = ISO3, weights = link)
```
