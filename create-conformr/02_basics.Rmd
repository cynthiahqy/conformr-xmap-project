...
Write this data into the package for testing purposes.
```{r internal-data-in}
equal_pm$data_A <- data_in |>
  dplyr::select(code_in = std_A,
                value_in = A_100)
...
#' 
has_bad_weights <- function(.data, code_in, code_out, weights){
  bad_rows <- .data |>
...
#' 
is_covered <- function(data_in, pm, code_in, code_out){
  missing_links <- data_in %>%
    dplyr::distinct({{code_in}}) %>%
    dplyr::antijoin(pm, by = code_in)
...
check_coverage <- function(data_in, pm, code_in, code_out, values_from){
  is_results <- is_covered(data_in, pm, {{code_in}}, {{code_out}})
}
```
